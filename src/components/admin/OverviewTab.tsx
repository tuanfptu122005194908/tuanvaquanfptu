...
import { ShoppingBag, DollarSign, Users, Clock, TrendingUp, ArrowUpRight, Package, Calendar } from "lucide-react";
...
const OverviewTab = ({ stats, orders, isLoading }: OverviewTabProps) => {
  const statCards = [
    ...
    {
      icon: Calendar,
      label: "Doanh thu kỳ học",
      value: `${((stats?.termRevenue || 0) / 1000000).toFixed(1)}M₫`,
        ...
        <Card className="border bg-card">
          ...
          <CardContent className="space-y-4">
            {stats?.currentTerm && (
              <div className="p-3 bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg border border-purple-200">
                <div className="flex items-center justify-between mb-2">
                  <span className="text-sm font-medium text-purple-700">Kỳ học hiện tại</span>
                  <Badge variant="outline" className="text-xs bg-purple-100 text-purple-700 border-purple-200">
                    Đang hoạt động
                  </Badge>
                </div>
                <p className="font-semibold text-purple-900">{stats.currentTerm.name}</p>
                <p className="text-xs text-purple-600">
                  Bắt đầu: {new Date(stats.currentTerm.start_date).toLocaleDateString('vi-VN')}
                </p>
              </div>
            )}
            <div className="space-y-3">
              ...
              <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
                <span className="text-sm text-muted-foreground">Đơn kỳ này</span>
                <span className="font-semibold text-purple-600">
                  {stats?.termOrders || 0} đơn
                </span>
              </div>
              <div className="flex items-center justify-between p-3 bg-muted/50 rounded-lg">
