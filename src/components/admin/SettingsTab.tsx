...
import { RefreshCw, Save, CreditCard, QrCode, Settings, Check, User, Lock, Eye, EyeOff, Shield, Image } from "lucide-react";
...
const SettingsTab = () => {
  ...
  const { uploadImage: uploadBanner, isUploading: isUploadingBanner } = useImageUpload({
    bucket: "site-assets",
    folder: "banners",
  });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (error) throw error;
      toast.success("Đã lưu cài đặt QR thanh toán!");
    } catch (error: any) {
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="payment" className="space-y-6">
        <TabsList className="bg-muted p-1 h-auto">
          ...
          <TabsTrigger value="banner" className="gap-2 data-[state=active]:bg-card data-[state=active]:shadow-sm">
          ...
          <Card>
            <CardHeader className="border-b bg-muted/30">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <CardTitle className="text-lg">Quản lý Banner</CardTitle>
                  <CardDescription>Cài đặt banner hiển thị trên trang chủ</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="p-6 space-y-6">
              <div className="p-4 bg-muted/50 rounded-xl border">
                <p className="text-sm text-muted-foreground mb-4">
                  Tải lên ảnh banner để hiển thị trên trang chủ. Kích thước đề nghị: 1920x400px
                </p>
                <ImageUpload
              ...
              {settings.banner_url && (
                <div className="p-6 bg-muted/30 rounded-xl border">
                  <div className="flex items-center gap-2 mb-4">
                    <Check className="h-4 w-4 text-success" />
                    <p className="text-sm font-medium text-success">Xem trước Banner</p>
                  </div>
